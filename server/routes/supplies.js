...
router.get("/category/:categoryName", async (req, res, next) => {
  ...
  const supplies = await Supply.findAll({
    ...
    order: [["name"], [{ model: Classroom }, "name"]],
  });
  if (!supplies.length) {
    let err = {
      name: `not found`,
...
router.get("/scissors/calculate", async (req, res, next) => {
  ...
  const scissors = await Supply.findAll({
    attributes: [
      [
        Sequelize.literal(
          "SUM(CASE WHEN `handed` = 'right' THEN 1 ELSE 0 END)"
        ),
        "numRightyScissors",
      ],
      [
        Sequelize.literal("SUM(CASE WHEN `handed` = 'left' THEN 1 ELSE 0 END)"),
        "numLeftyScissors",
      ],
      [Sequelize.literal("COUNT(`handed`)"), "totalNumScissors"],
    ],
    where: {
      name: {
        [Op.like]: "%scissors%",
  ...
  // Your code here
  const studentsInAllClassrooms = await Classroom.findAll({
    attributes: [],
    include: {
      model: Student,
      attributes: [
        [
          Sequelize.fn(
            "SUM",
            Sequelize.literal("CASE WHEN `leftHanded` = 1 THEN 1 ELSE 0 END")
          ),
        ...
        [
          Sequelize.fn(
            "SUM",
            Sequelize.literal("CASE WHEN `leftHanded` = 0 THEN 1 ELSE 0 END")
          ),
  ...
  // Your code here
  const jsonStudents = studentsInAllClassrooms.map((item) => item.toJSON());

  const students = jsonStudents[0].Students;
  console.log(scissors[0].numLeftyScissors);
  console.log(students);
  const stillNeededScissors = {
    numRightyScissorsStillNeeded:
      students[0].numRightHandedStudents - scissors[0].numRightyScissors,
    numLeftyScissorsStillNeeded:
      students[0].numLeftHandedStudents - scissors[0].numLeftyScissors,
  };

  result = [...scissors, ...students, stillNeededScissors];
  res.json(result);
});
